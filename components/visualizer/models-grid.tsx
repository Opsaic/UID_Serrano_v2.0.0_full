...
export async function ModelsGrid() {
  try {
    ...
    const { data: models, error } = await supabase.from("models").select("*").order("created_at", { ascending: false })
    ...
    console.error("[v0] ModelsGrid error:", error)
    return (
      <div className="col-span-full text-center py-12">
        <p className="text-destructive mb-4">Error loading 3D models</p>
